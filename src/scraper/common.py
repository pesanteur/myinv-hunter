...
class ScrapeResult(ABC):
    ...
    def set_price(self, tag):
        ...
        if not re_natch:
            self.logger.warning(f'unabte to find price in string: "{price_str}"')
            return
...
class Scraper(ABC): # research from abc import ABC, abstractmethod
    def __init__(self, drivers, url):
        ...
        self.logger = loggin.getLogger(url.nickname)
        self.url = url
        self.last_result = None
        self.logger.info(f'scraper initialized for {self.url}')

    @staticmethod # research how these methods work
    @abstractmethod
    def get-domain():
        pass
